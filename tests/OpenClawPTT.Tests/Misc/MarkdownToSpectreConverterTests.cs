...
public class MarkdownToSpectreConverterTests
{
    ...
    public void Convert_InlineCode_YieldsBoldYellow()
        => Assert.Equal("[bold yellow]code[/]", MarkdownToSpectreConverter.Convert("`code`"));

    [Fact]
    public void Convert_InlineCode_MultipleBackticks()
        => Assert.Equal("[bold yellow]a[/] and [bold yellow]b[/]", MarkdownToSpectreConverter.Convert("`a` and `b`"));
    ...
    [Fact]
    public void Convert_FencedCodeBlock_YieldsDimLines()
    {
        var md = "```\nlet x = 1;\n```";
        var result = MarkdownToSpectreConverter.Convert(md).Replace("\r\n", "\n");
        Assert.Contains("[dim]let x = 1;[/]", result);
    }

    [Fact]
    public void Convert_FencedCodeBlockWithLanguage_YieldsDimLines()
    {
        var md = "```javascript\nconsole.log('hi');\n```";
        var result = MarkdownToSpectreConverter.Convert(md).Replace("\r\n", "\n");
        Assert.Contains("[dim]console.log('hi');[/]", result);
    }

    [Fact]
    public void Convert_FencedCodeBlock_EmptyBlock_YieldsNoOutput()
    {
        var md = "```\n```";
        var result = MarkdownToSpectreConverter.Convert(md);
        // Should not contain any [dim] lines for content (fence itself is skipped)
        Assert.DoesNotContain("[dim]", result);
    }
    ...
    public void Convert_CompoundMarkdown_AllConstructs()
    {
        ...
        Assert.Contains("[bold yellow]code[/]", result);
